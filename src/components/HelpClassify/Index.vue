<template>
  <div class="index-page" v-show="power['boss_check_help_classify']">
    <el-button v-show="power['boss_create_help_classify']" @click="addOrUpdate()" type="warning">{{$t('新增')}}
    </el-button>
    <el-button v-show="power['boss_delete_help_classify']" @click="remove()" type="warning">{{$t('删除')}}
    </el-button>
    <el-button v-show="power['boss_release_help_classify']" @click="release(1)" type="warning">{{$t('发布')}}
    </el-button>
    <el-button style="margin-bottom: 5px;" v-show="power['boss_release_help_classify']" @click="release(0)"
      type="warning">
      {{$t('取消发布')}}
    </el-button>
    ...
    <div class="form-check">
      <label>
        {{$t('分类名称(中文)')}}：
        <div>
          <el-input @keyup.enter.native="search" v-model="params.type_zh" :placeholder="$t('模糊搜索')"></el-input>
        </div>
      </label>
    ...
    <el-table class="select-table" ref="multipleTable" :data="list" border tooltip-effect="dark" style="width: 100%;"
      ...
      <el-table-column :label="$t('分类名称(英语)')" show-overflow-tooltip prop="type_en" min-width="120">
      </el-table-column>

      <el-table-column :label="$t('分类名称(葡语)')" show-overflow-tooltip prop="type_pt" min-width="120">
      </el-table-column>

      <el-table-column :label="$t('分类名称(西语)')" show-overflow-tooltip prop="type_es" min-width="120">
      </el-table-column>
      ...
      <el-table-column :label="$t('操作')" show-overflow-tooltip fixed="right" min-width="80">
        <template scope="scope">
          <el-button v-show="power['boss_update_help_classify']" @click="addOrUpdate(scope.row)" type="text"
    ...
    <div class="model-heard" style="position:relative;">
      <el-dialog :title="title" v-model="dialogTableVisible">
        <Add :detailsObj="detailsObj" :addTime="addTime" v-on:message="search" :typeOptions="typeOptions"
