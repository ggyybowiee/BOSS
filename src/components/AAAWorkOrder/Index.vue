<template>
  <div class="index-page" v-show="power['boss_check_aaawk']">

    <el-button v-show="power['boss_resend_aaawk']" @click="resend()" type="warning">{{$t('重发')}}
    </el-button>
    ...
    <el-table class="select-table" ref="multipleTable" :data="list" border tooltip-effect="dark" style="width: 100%"
      ...
      <el-table-column :label="$t('操作')" show-overflow-tooltip min-width="80">
        <template scope="scope">
          <el-button v-show="power['boss_resend_aaawk'] && scope.row.requestStatus==3" @click="resend(scope.row)"
