  ...
  methods: {
    ...
    reset_search() {
      ...
      for (let key in _self[type].params) {
        if (["packageName", "packageCode", "status"].indexOf(key) > -1) {
      ...
      this.$http
        ...
        .then((response) => {
          ...
          if (response.data.returnCode == 0) {
            _self[_self.activeName].list = response.data.packageInfoList;
          } else {
