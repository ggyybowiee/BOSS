<template>
  <div class="index-page" v-show="power['boss_check_product']">
    <el-button v-show="power['boss_create_product']" @click="addOrUpdate()" type="warning">{{$t('新增产品')}}
    </el-button>
    <el-button style="margin-bottom: 5px;" v-show="power['boss_synchro_product']" @click="synchro()" type="warning">
      {{$t('同步')}}
    </el-button>
    <div class="form-check">
      <label>
        {{$t('产品名称')}}：
        <div>
          <el-input @keyup.enter.native="search" v-model="params.productName" :placeholder="$t('模糊搜索')"></el-input>
        </div>
      </label>
      ...
      <label class="product-code">
        {{$t('产品编码')}}：
        <div>
          <el-input @keyup.enter.native="search" v-model="params.productCode" :placeholder="$t('精确搜索')"></el-input>
        </div>
      </label>
    ...
    <el-table class="select-table" ref="multipleTable" :data="list" border tooltip-effect="dark" style="width: 100%;"
      ...
      <el-table-column :label="$t('产品编码')" show-overflow-tooltip prop="productCode" min-width="200"></el-table-column>
      ...
      <el-table-column :label="$t('操作')" show-overflow-tooltip fixed="right" min-width="80">
        <template scope="scope">
          <el-button @click="link(scope.row)" type="text" size="small">
            {{$t('关联')}}</el-button>
          <el-button v-show="power['boss_update_product']" @click="addOrUpdate(scope.row)" type="text" size="small">
            {{$t('修改')}}
          </el-button>
        </template>
      </el-table-column>
